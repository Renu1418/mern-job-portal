        ...
        <div>
          <h1 className='font-bold text-xl'>{singleJob?.title}</h1>
                <div className ='flex items-center gap-2 mt-4'>
                  <Badge variant="outline" className="bg-[#F3EEFF] text-[#6A38C2] border-0 font-medium" >{singleJob?.position} Positions</Badge>
                  <Badge variant="outline" className="bg-[#ECFDF5] text-[#16A34A] border-0 font-medium" >{singleJob?.jobType}</Badge>
                  <Badge variant="outline" className="bg-[#FFF7ED] text-[#EA580C] border-0 font-medium" >{singleJob?.salary}</Badge>
                </div>
        </div>

        <Button onClick= {isInitiallyApplied ? null : applyJobHandler}
          disabled={isInitiallyApplied}
          className={`rounded-lg ${isInitiallyApplied ? 'bg-gray-600 cursor-not-allowed' : 'bg-[#7209b7] hover:bg-[#5f0aa0]'}`
